...
import pkg/sdl2/[gamecontroller, image, ttf], pkg/[seiryu/dbc]
import ./[exceptions, saohime_types]
...
proc createRect(position, size: Vector): Rect =
  return rect(position.x.cint, position.y.cint, size.x.cint, size.y.cint)

proc createRectF(position, size: Vector): RectF =
  return rectf(position.x, position.y, size.x, size.y)
...
proc createWindow*(
  ...
  result =
    sdl2.createWindow(title.cstring, x.cint, y.cint, width.cint, height.cint, flags)
...
proc createRenderer*(
    window: WindowPtr, index: int, flags: cint
): RendererPtr {.raises: [SDL2RendererError].} =
  precondition:
    window != nil

  result = sdl2.createRenderer(window, index.cint, flags)
...
proc setTarget*(
    renderer: RendererPtr, texture: TexturePtr
...
proc setTargetToDefault*(renderer: RendererPtr) {.raises: [SDL2RendererError].} =
  precondition:
    renderer != nil
...
proc setViewport*(
    renderer: RendererPtr, position1, position2: Vector
...
proc setDrawBlendMode*(
    renderer: RendererPtr, blendMode: BlendMode
) {.raises: [SDL2DrawError].} =
  precondition:
    renderer != nil
...
proc setDrawColor*(
    renderer: RendererPtr, color: SaohimeColor
) {.raises: [SDL2DrawError].} =
  precondition:
    renderer != nil
...
proc setScale*(renderer: RendererPtr, scale: Vector) {.raises: [SDL2DrawError].} =
  precondition:
    renderer != nil
...
proc drawPoint*(renderer: RendererPtr, position: Vector) {.raises: [SDL2DrawError].} =
  precondition:
    renderer != nil
...
proc drawLine*(
  ...
  let exitCode = renderer.drawLineF(position1.x, position1.y, position2.x, position2.y)
  raiseError(exitCode == SdlError):
    let msg = "Failed to draw point: " & $sdl2.getError()
    raise (ref SDL2DrawError)(msg: msg)

proc drawRectangle*(
    renderer: RendererPtr, position, size: Vector
) {.raises: [SDL2DrawError].} =
  precondition:
    renderer != nil
...
proc loadTexture*(
    renderer: RendererPtr, file: string
): TexturePtr {.raises: [SDL2TextureError].} =
  precondition:
    renderer != nil

  result = loadTexture(renderer, file.cstring)
...
proc createTexture*(
    renderer: RendererPtr, format: uint32, access: cint, width, height: int
): TexturePtr =
  return renderer.createTexture(format, access, w = width.cint, h = height.cint)

proc createTextureFromSurface*(
    renderer: RendererPtr, surface: SurfacePtr
): TexturePtr {.raises: [SDL2TextureError].} =
  precondition:
    renderer != nil
...
proc copy*(
  ...
  var
    sourceRect = createRect(source.position, source.size)
    destinationRect = createRectF(destination.position, destination.size)

  let exitCode = sdl2.copyf(renderer, texture, addr sourceRect, addr destinationRect)
  raiseError(exitCode == SdlError):
...
proc openFont*(file: string, fontSize: int): FontPtr {.raises: [SDL2FontError].} =
...
proc renderTextBlended*(
    font: FontPtr, text: string, fg: SaohimeColor
...
proc openController*(index: int): GameControllerPtr {.raises: [SDL2InputError].} =
